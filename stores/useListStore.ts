...
const useListStore = create<ListStore>((set, get) => ({
    ...
    try {
      ...
      if (success) {
        set(state => ({
          ...
          currentItems: state.currentList?.id === id ? [] : state.currentItems,
        }));
      }
    } catch (error) {
      set({
  ...
  selectList: async (id: string) => {
    ...
    try {
      ...
      set({
        error: error instanceof Error ? error.message : 'Failed to select list',
        isLoading: false,
      });
    }
  },
  ...
  deleteCheckedItems: async (listId: string) => {
    set({ error: null });
    try {
      const deletedCount = await listItemRepository.deleteCheckedItems(listId);
